...
\begin{itemize}
    \item \textbf{Precision:} \( \boxed{ 0.6667 } \)
    \item \textbf{Recall:} \( \boxed{ 0.8 } \)
...
\begin{align*}
    Precision &= \frac{30}{30 + 10} \\
    &= \frac{30}{40} \\
    Precision &= 0.75 \\
    Recall &= \frac{30}{30 + 20} \\
    &= \frac{30}{50} \\
    Recall &= 0.6 \\
    F1 &= \frac{(2 \times 30)}{(2 \times 30) + 10 + 20} \\
    &= \frac{60}{60 + 30} \\
    &= \frac{60}{90} \\
    F1 &\approx 0.6667
\end{align*}
...
Thus, the calculated metrics are:
\begin{itemize}
    \item \textbf{Precision:} \( \boxed{ 0.75 } \)
...
\hspace{4mm} Next, we calculate the accuracy, precision, recall, and F1 score:
...
\begin{solution}
First, we start with the initial centroids:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T5_iter_0.png}
\end{center}

Apply the K-means algorithm step by step with the given starting centroids \( (3, 3) \), \( (2, 2) \), and \( (-3, -3) \).
The steps are as follows:
\[
\renewcommand{\arraystretch}{1.3}
\begin{array}{|c|c|c|}
    \hline
    x & y & \text{Assigned Centroid} \\
    \hline
    1 & 2 & (2, 2) \\
    3 & 3 & (3, 3) \\
    2 & 2 & (2, 2) \\
...
\begin{itemize}
    \item For centroid \( (3, 3) \):
    \[ \text{New Centroid} = \paren{ \frac{3 + 8 + 7 + 6}{4}, \frac{3 + 6 + 8 + 7}{4} } = (6, 6) \]
    \item For centroid \( (2, 2) \):
    \[ \text{New Centroid} = \paren{ \frac{1 + 2}{2}, \frac{2 + 2}{2} } = (1.5, 2) \]
    \item For centroid \( (-3, -3) \):
    \[ \text{New Centroid} = \paren{ \frac{-3 + -2 + -7}{3}, \frac{-3 + -4 + -7}{3} } = (-4, -4.67) \]
\end{itemize}

After the first iteration, the updated centroids are:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T5_iter_1.png}
\end{center}

Continuing the K-means algorithm with the new centroids \( (6, 6) \), \( (1.5, 2) \), and \( (-4, -4.67) \), we got:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T5_iter_2.png}
\end{center}

From the second iteration, the centroids do not change anymore, indicating convergence.
Thus, the final centroids after convergence are:
\begin{itemize}
    \item Centroid 1: \( (6, 6) \)
    \item Centroid 2: \( (1.5, 2) \)
    \item Centroid 3: \( (-4, -4.67) \)
\end{itemize}
...
\begin{solution}
Just like in Problem T05, we start with the initial centroids:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T6_iter_0.png}
\end{center}

Applying the K-means algorithm step by step with the given starting centroids \( (-3, -3) \), \( (2, 2) \), and \( (-7, -7) \).
The first iteration results shown below:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T6_iter_1.png}
\end{center}

After the first iteration, the updated centroids are:
\begin{itemize}
    \item For centroid \( (-3, -3) \):
    \[ \text{New Centroid} = \paren{ \frac{-3 + -2}{2}, \frac{-3 + -4}{2} } = (-2.5, -3.5) \]
    \item For centroid \( (2, 2) \):
    \[ \text{New Centroid} = \paren{ \frac{1 + 3 + 2 + 8 + 7 + 6}{6}, \frac{2 + 3 + 2 + 6 + 8 + 7}{6} } = (4.5, 4.67) \]
    \item For centroid \( (-7, -7) \):
    \[ \text{New Centroid} = (-7, -7) \] (a point itself)
\end{itemize}

Continuing the K-means algorithm with the new centroids \( (-2.5, -3.5) \), \( (4.5, 4.67) \), and \( (-7, -7) \), we got:
\begin{center}
    \includegraphics[width=0.7\textwidth]{images/centroid_movement_T6_iter_2.png}
\end{center}

From the second iteration, the centroids do not change anymore, indicating convergence.
Thus, the final centroids after convergence are:
\begin{itemize}
    \item Centroid 1: \( (-2.5, -3.5) \)
    \item Centroid 2: \( (4.5, 4.67) \)
    \item Centroid 3: \( (-7, -7) \)
\end{itemize}
...
\begin{solution}
To determine which set of starting points is better between the two previous questions,
we can consider the following criteria for measuring the `goodness' of a set of starting points:
\begin{itemize}
    \item \textbf{Convergence Speed:} A better set of starting points should lead to faster convergence of the K-means algorithm.
    This can be measured by the number of iterations required to reach convergence.
    \item \textbf{Cluster Compactness:} The resulting clusters should be compact and well-separated.
    This can be measured using metrics such as the Within-Cluster Sum of Squares (WCSS) or Silhouette Score.
    \item \textbf{Stability:} A good set of starting points should lead to consistent clustering results across multiple runs of the algorithm.
    This can be evaluated by running the K-means algorithm multiple times with different random initializations and measuring the variance in the resulting clusters.
    \item \textbf{Interpretability:} The resulting clusters should be interpretable and meaningful in the context of the data.
    This can be assessed qualitatively by examining the characteristics of the data points within each cluster.
\end{itemize}

\hspace{4mm} In the previous two questions, we can analyze the results based on these criteria.
In Problem T05, the starting points led to convergence in \textbf{2 iterations}, while in Problem T06, it also converged in \textbf{2 iterations}.

\hspace{4mm} However, the clusters formed in Problem T05 appear to be \textbf{more compact} and \textbf{well-separated} compared to those in Problem T06.

\hspace{4mm} Therefore, based on the criteria mentioned above, we can conclude that:
\[ \boxed{ \textbf{the starting points in Problem T05 are better} } \]
\end{solution}
% ================================================================================ %
...
\begin{solution}
To determine the best value of K for the given dataset, we can consider several approaches and reasoning:
\begin{itemize}
    \item \textbf{Elbow Method:} This method involves plotting the Within-Cluster Sum of Squares (WCSS) against different values of K.
    The idea is to look for an `elbow' point in the plot where the rate of decrease in WCSS starts to slow down.
    This point indicates a good balance between cluster compactness and the number of clusters.
    \item \textbf{Silhouette Score:} This metric measures how similar a data point is to its own cluster compared to other clusters.
    A higher silhouette score indicates better-defined clusters.
    We can calculate the silhouette score for different values of K and choose the one that maximizes the score.
    \item \textbf{Domain Knowledge:} If we have prior knowledge about the data, we can use that information to guide our choice of K.
    For example, if we know that the data represents three distinct categories, we might choose K=3.
    \item \textbf{Visual Inspection:} For low-dimensional data, we can visualize the clusters formed by different values of K and choose the one that appears to best capture the underlying structure of the data.
\end{itemize}

\hspace{4mm} In the case of the given dataset, we can start by applying the Elbow Method and Silhouette Score to evaluate different values of K.
Based on the visual inspection of the data points, it appears that there are three distinct clusters:
\begin{itemize}
    \item A cluster around the points \( (1, 2) \), \( (2, 2) \), and \( (3, 3) \).
    \item A cluster around the points \( (6, 7) \), \( (7, 8) \), and \( (8, 6) \).
    \item A cluster around the points \( (-3, -3) \), \( (-2, -4) \), and \( (-7, -7) \).
\end{itemize}

Therefore, based on the analysis and reasoning above, we can conclude that:
\[ \boxed{ \textbf{the best value of K is 3} } \]
\end{solution}
% ================================================================================ %
...
\begin{center}
    \includegraphics[width=0.38\textwidth]{images/my_heart_will_go_on.png}
\end{center}
...
\begin{solution}
Using the Pandas library and the code provided above,
we find that the median age of the training set is approximately \( \boxed{28.0} \) years.
\end{solution}
% ================================================================================ %
...
\begin{solution}
Using the Pandas library, we can write the following code to find the mode of the 'Embarked' column and fill in the missing values:
\begin{codingbox}
...
After running the code, we find that the mode of the 'Embarked' column is \( \boxed{S} \).
We then fill the missing values in the 'Embarked' column with this mode value.

\hspace{4mm} Finally, we convert the categorical values to numerical values using the following dictionary mapping:
\begin{codingbox}
...
% ================================================================================ %
\begin{Tproblem}
Write a linear regression classifier using gradient descent as learned in class.
...
\begin{solution}
After extracting the features from the Pandas DataFrame to a Numpy array and forcing the data type to float,
we can implement a linear regression classifier using gradient descent.

\hspace{4mm} The final \( \theta \) (parameters) is:
\begin{codingbox}
theta = [0.023536, 0.663266, 0.000802, 0.188622]
\end{codingbox}

With these parameters, we can predict the survival of passengers based on the input features.
The MSE on the training set is approximately \( \boxed{0.185767} \).
\end{solution}
% ================================================================================ %
...
\begin{solution}
After implementing the linear regression classifier and making predictions on the test set,
we submit the results to Kaggle.
The screenshot of the submission with the scores is as follows:
\begin{center}
    \includegraphics[width=0.5\textwidth]{images/kaggle_submission.png}
\end{center}

The score obtained from the submission is \( \boxed{0.76555} \) (accuracy).
\end{solution}
% ================================================================================ %
...
\begin{solution}
After adding higher order features such as \( \texttt{Pclass} \times \texttt{Embarked} \) to the training set,
we retrain the linear regression classifier using gradient descent.

The final \( \theta \) (parameters) after adding higher order features is:
\begin{codingbox}
theta = [0.048699, 0.66859, 0.000802, 0.238304, -0.092839]
\end{codingbox}

\hspace{4mm} With these new parameters, we evaluate the model on both the training and test sets.
The MSE on the training set is approximately \( \boxed{0.174827} \),
indicating an improvement in accuracy compared to the previous model.

\hspace{4mm} On the test set, the accuracy obtained from the submission is \( \boxed{0.76555} \),
which is the same as the previous model without higher order features.

\hspace{4mm} Thus, while adding higher order features improved the accuracy on the training set,
it did not lead to an improvement in accuracy on the test set.

\hspace{4mm} The results can be summarized as follows:
\begin{itemize}
    \item Training Set MSE: \( \boxed{0.174827} \)
    \item Test Set Accuracy: \( \boxed{0.76555} \)
\end{itemize}

The submission screenshot is as follows:
\begin{center}
    \includegraphics[width=0.5\textwidth]{images/kaggle_submission_higher_order.png}
\end{center}
\end{solution}
% ================================================================================ %
...
\begin{solution}
Likewise, we reduce the input features to just \texttt{Sex} and \texttt{Age},
and retrain the linear regression classifier using gradient descent.

The final \( \theta \) (parameters) after reducing the features is:
\begin{codingbox}
theta = [0.455894, 0.006178]
\end{codingbox}

\hspace{4mm} With these new parameters, we evaluate the model on both the training and test sets.
The MSE on the training set is approximately \( \boxed{0.179285} \),
indicating a decrease in accuracy compared to the previous models.

\hspace{4mm} On the test set, the accuracy obtained from the submission is \( \boxed{0.76315} \),
which is slightly lower than the previous models.

\hspace{4mm} Thus, reducing the input features to just \texttt{Sex} and \texttt{Age} led to a decrease in accuracy on both the training and test sets.

\newpage

\hspace{4mm} The results can be summarized as follows:
\begin{itemize}
    \item Training Set MSE: \( \boxed{0.179285} \)
    \item Test Set Accuracy: \( \boxed{0.76315} \)
\end{itemize}

The submission screenshot is as follows:
\begin{center}
    \includegraphics[width=0.5\textwidth]{images/kaggle_submission_reduced_features.png}
\end{center}
\end{solution}
% ================================================================================ %
...
\begin{solution}
This have already been done in Problem T10.
...
The accuracy obtained from the submission is \( \boxed{0.76555} \)
\end{solution}
% ================================================================================ %
...
\begin{solution}
After implementing the linear regression classifier using matrix inversion,
\[ \theta = \paren{ X^T X }^{-1} X^T y \]

The final \( \theta \) (parameters) obtained from matrix inversion is:
\begin{codingbox}
theta = [-0.014114, 0.604206, 0.005015, 0.061163]
\end{codingbox}

\hspace{4mm} To compare the weights learned from the two methods, we calculate the Mean Squared Error (MSE) between the weights obtained from gradient descent and matrix
inversion:
\begin{codingbox}
mse_difference = np.mean((theta_gradient_descent - theta_matrix_inversion) ** 2)
\end{codingbox}

The MSE of the difference between the two weights is approximately \( \boxed{0.005292} \).
Which indicated that, even the weights learned from the two methods are not identical,
they are quite similar, as indicated by the low MSE value.

\hspace{4mm} The accuracy obtained from the submission using matrix inversion is \( \boxed{0.76555} \),
which is exactly the same as the previous model using gradient descent.

\begin{center}
    \includegraphics[width=0.5\textwidth]{images/kaggle_submission_matrix_inversion.png}
\end{center}

\vspace{3mm}

\hspace{4mm} Thus, we can conclude that both methods yield similar results in terms of accuracy on the test set,
and the weights learned from both methods are quite similar as indicated by the low MSE value.
\end{solution}
% ================================================================================ %
...
\begin{proofbox}
Given the function \( f(A) = tr(AB) \), we want to find the gradient of \( f \) with respect to the matrix \( A \).
First, we can express the trace of the product \( AB \) in terms of the elements of the matrices:
\[ tr(AB) = \sum_{i} (AB)_{i,i} = \sum_{i} \sum_{j} A_{i,j} B_{j,i} \]

To find the gradient \( \nabla_A f(A) \), we need to compute the partial derivative of \( f \) with respect to each element \( A_{k,l} \):
\[ \frac{\partial f}{\partial A_{k,l}} = \frac{\partial}{\partial A_{k,l}} \sum_{i} \sum_{j} A_{i,j} B_{j,i} \]
...
Thus, the gradient \( \nabla_A f(A) \) is given by the matrix whose \( (k,l) \) element is \( B_{l,k} \), which is precisely \textbf{the transpose} of matrix \( B \):
\[ \nabla_A f(A) = B^T \]
...
\begin{proofbox}
Let \( f(A) \) be a scalar function of the matrix \( A \).
To prove this, we start by expressing the elements of the gradients.
The \( (i,j) \) element of the gradient \( \nabla_A f(A) \) is given by:
...
\begin{proofbox}
Let \( f(A) = tr(AB A^T C) \). We want to find the gradient of \( f \) with respect to the matrix \( A \).
First, we can express the trace in terms of the elements of the matrices:
\[ tr(AB A^T C) = \sum_{i} (AB A^T C)_{i,i} = \sum_{i} \sum_{j} \sum_{k} \sum_{l} A_{i,j} B_{j,k} A^T_{k,l} C_{l,i} \]

Since \( A^T_{k,l} = A_{l,k} \), we can rewrite the expression as:
\[ tr(AB A^T C) = \sum_{i} \sum_{j} \sum_{k} \sum_{l} A_{i,j} B_{j,k} A_{l,k} C_{l,i} \]

To find the gradient \( \nabla_A f(A) \), we need to compute the partial derivative of \( f \) with respect to each element \( A_{p,q} \):
\[ \frac{\partial f}{\partial A_{p,q}} = \frac{\partial}{\partial A_{p,q}} \sum_{i} \sum_{j} \sum_{k} \sum_{l} A_{i,j} B_{j,k} A_{l,k} C_{l,i} \]

The only terms in the triple sum that depend on \( A_{p,q} \) are those where \( i = p \) and \( j = q \) or \( l = p \) and \( k = q \).
Thus, we have:
\begin{align*}
    \frac{\partial f}{\partial A_{p,q}} &= \frac{\partial}{\partial A_{p,q}} \sum_{i} \sum_{j} \sum_{k} \sum_{l} A_{i,j} B_{j,k} A_{l,k} C_{l,i} \\
    &= \frac{\partial}{\partial A_{p,q}} \sqbracket{ \sum_{k} \sum_{l} A_{p,q} B_{q,k} A_{l,k} C_{l,p} + \sum_{i} \sum_{j} A_{i,j} B_{j,q} A_{p,q} C_{p,i} } \\
    &= \frac{\partial}{\partial A_{p,q}} A_{p,q} \sqbracket{ \sum_{k} \sum_{l} B_{q,k} A_{l,k} C_{l,p} + \sum_{i} \sum_{j} A_{i,j} B_{j,q} C_{p,i} } \\
    &= \sum_{k} \sum_{l} B_{q,k} A_{l,k} C_{l,p} + \sum_{i} \sum_{j} A_{i,j} B_{j,q} C_{p,i} \\
    &= \sum_{k} \sum_{l} B_{q,k} A^T_{k,l} C_{l,p} + \sum_{i} \sum_{j} C_{p,i} A_{i,j} B_{j,q} \\
    &= (B A^T C)_{q,p} + (C A B)_{p,q} \\
    &= (B A^T C)^T_{p,q} + (C A B)_{p,q} \\
    &= (C^T A B^T + C A B)_{p,q} \\
    \frac{\partial f}{\partial A_{p,q}} &= (C A B + C^T A B^T)_{p,q}
\end{align*}

Thus, the gradient \( \nabla_A f(A) \) is given by the matrix whose \( (p,q) \) element is \( (C A B + C^T A B^T)_{p,q} \):
\[ \nabla_A f(A) = C A B + C^T A B^T \]
\end{proofbox}
% ================================================================================ %
...
\myappendix{code/clustering.pdf}{Clustering Code}
\myappendix{code/my-heart-will-go-on.pdf}{My Heart Will Go On Code (Kaggle Titanic)}
